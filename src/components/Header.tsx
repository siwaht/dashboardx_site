...
export default function Header() {
  ...
  return (
    <header className="sticky top-0 z-50 py-5 md:py-7 bg-light-primary/90 dark:bg-dark-primary/90 backdrop-blur-2xl border-b border-brand-slate-light/30 dark:border-brand-slate/30 transition-all duration-500 shadow-soft">
      <div className="container mx-auto px-4 md:px-8">
        <nav className="flex justify-between items-center">
          <a
            href="#"
            className="font-display text-2xl md:text-3xl font-extrabold text-gradient transition-all hover:scale-105 inline-block duration-500 ease-bounce-soft"
          ...
          <ul className="hidden md:flex gap-6 lg:gap-8 items-center">
            ...
            <li>
              <button
                onClick={toggleTheme}
                className="group p-3 rounded-xl bg-light-secondary/90 dark:bg-dark-secondary/90 hover:scale-110 transition-all duration-500 ease-bounce-soft min-w-[48px] min-h-[48px] flex items-center justify-center shadow-soft hover:shadow-soft-lg border border-brand-slate-light/30 dark:border-brand-slate/30 hover:border-brand-blue-mid/50 dark:hover:border-brand-amber-mid/50"
                aria-label={theme === 'light' ? 'Switch to dark mode' : 'Switch to light mode'}
              >
                {theme === 'light' ? (
                  <Moon className="w-6 h-6 text-brand-blue-mid group-hover:rotate-12 transition-transform duration-500" />
                ) : (
                  <Sun className="w-6 h-6 text-brand-amber-mid group-hover:rotate-90 transition-transform duration-500" />
                )}
              </button>
            </li>
            <li>
              <a
                href="#demo-form"
                className="btn-ripple inline-block bg-gradient-elegant text-white px-8 py-4 rounded-full font-bold hover:-translate-y-2 hover:shadow-soft-lg hover:scale-105 transition-all duration-500 ease-bounce-soft min-h-[48px] flex items-center relative overflow-hidden group shadow-soft border border-white/20"
